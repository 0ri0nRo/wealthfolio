...
def classify(paths):
    required = dict(frontend=False, rust=False, formatting=False, android=False,
                    ios=False, android_build=False, ios_build=False)
    for path in paths:
        if path.startswith((".github/workflows/", ".github/scripts/")):
            ...
            required.update(rust=True, android=True, android_build=True)
            ...
            required.update(rust=True, ios=True, ios_build=True)
            ...
            if not path.startswith("apps/server/"):
                ...
                # Native integration, dependency and TLS changes also need linking/packaging.
                if (path.startswith(("apps/tauri/", ".cargo/", "crates/http/"))
                        or path.endswith(("/Cargo.toml", "/build.rs"))
                        or path in ("Cargo.toml", "Cargo.lock", "rust-toolchain", "rust-toolchain.toml")):
                    required.update(android_build=True, ios_build=True)
            ...
            if path in ("package.json", "pnpm-lock.yaml", "pnpm-workspace.yaml", "apps/frontend/package.json"):
                required.update(android=True, ios=True, android_build=True, ios_build=True)
    return required
