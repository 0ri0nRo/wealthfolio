...
export const BudgetPage: React.FC = () => {
  ...
  const statCards = [
    { label: 'Income',        value: totalIncome,   icon: <TrendingUp size={14} />,   iconColor: '#16a34a', iconBg: 'color-mix(in srgb, #16a34a 12%, var(--background))' },
    { label: 'Expenses',      value: expensesTotal, icon: <TrendingDown size={14} />, iconColor: '#dc2626', iconBg: 'color-mix(in srgb, #dc2626 12%, var(--background))' },
    { label: 'Savings',       value: investments,   icon: <TrendingUp size={14} />,   iconColor: '#7c3aed', iconBg: 'color-mix(in srgb, #7c3aed 12%, var(--background))' },
    {
      ...
      iconBg: cashBalance >= 0 ? 'color-mix(in srgb, #2563eb 12%, var(--background))' : 'color-mix(in srgb, #dc2626 12%, var(--background))',
    },
    {
      label: 'Meal Vouchers', value: bpBalance,
      icon: <UtensilsCrossed size={14} />,
      iconColor: bpBalance >= 0 ? '#d97706' : '#dc2626',
      iconBg: bpBalance >= 0 ? 'color-mix(in srgb, #d97706 12%, var(--background))' : 'color-mix(in srgb, #dc2626 12%, var(--background))',
    },
  ];
    ...
    <div style={{ minHeight: '100vh', background: 'var(--background)', fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Desktop: compact month selector */}
          {!isMobile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
              <button onClick={goToPrevMonth} style={{ width: 28, height: 28, border: 'none', borderRadius: '8px', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', WebkitTapHighlightColor: 'transparent' }}>
                <ChevronLeft size={15} />
              </button>
              <button
                onClick={() => setSelectedMonth(new Date())}
                style={{ padding: '4px 10px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700, whiteSpace: 'nowrap', letterSpacing: '-0.01em', WebkitTapHighlightColor: 'transparent', background: isCurrentMonth ? 'var(--muted)' : 'color-mix(in srgb, #d97706 15%, var(--background))', color: isCurrentMonth ? 'var(--foreground)' : '#d97706' }}
              >
                {selectedMonth.toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
              </button>
              <button onClick={goToNextMonth} style={{ width: 28, height: 28, border: 'none', borderRadius: '8px', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', WebkitTapHighlightColor: 'transparent' }}>
          ...
          <div ref={gearRef} style={{ position: 'relative' }}>
            <button
              ...
              style={{
                width: 34, height: 34, border: 'none', borderRadius: '10px', cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: showGearMenu || isBalanceHidden ? 'var(--foreground)' : 'var(--muted)',
                color:      showGearMenu || isBalanceHidden ? 'var(--background)' : 'var(--muted-foreground)',
                transition: 'all 0.15s',
                WebkitTapHighlightColor: 'transparent',
              }}
            >
            ...
            {showGearMenu && (
              <div style={{
                ...
                animation: 'dropIn 0.16s cubic-bezier(.34,1.4,.64,1) forwards',
              }}>
                {/* Hide balances row */}
                <div
      ...
      <div style={{
        maxWidth: 1280, margin: '0 auto',
        padding: isMobile
          ? `0 0 calc(var(--mobile-nav-ui-height, 64px) + max(var(--mobile-nav-gap, 8px), env(safe-area-inset-bottom)) + 1rem)`
          : '1.75rem 1.5rem',
      }}>

        {activeTab === 'overview' && (
          <>
            {/* Mobile: big month selector in content */}
            {isMobile && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '14px 1rem 10px' }}>
                ...
                <button onClick={() => setSelectedMonth(new Date())} style={{ border: 'none', background: 'transparent', cursor: 'pointer', WebkitTapHighlightColor: 'transparent', padding: 0 }}>
                  <span style={{ fontSize: '1.25rem', fontWeight: 800, letterSpacing: '-0.03em', color: isCurrentMonth ? 'var(--foreground)' : '#d97706' }}>
                    {selectedMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                  </span>
                  {!isCurrentMonth && (
                    <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#2563eb', marginLeft: '8px' }}>→ today</span>
                  )}
                </button>
            ...
            {!isMobile && (
              <>
                <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ width: 380, flexShrink: 0, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <div
                  ...
                  style={{ marginTop: '1.25rem', padding: '1rem 1.25rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', transition: 'background 0.15s', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'var(--accent)')}
                  onMouseLeave={e => (e.currentTarget.style.background = 'var(--card)')}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: 'color-mix(in srgb, #2563eb 12%, var(--background))', color: '#2563eb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <BarChart2 size={16} />
                    </div>
                    <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '0 1rem' }}>
                ...
                <div onClick={() => setActiveTab('yearly')} style={{ padding: '0.9rem 1rem', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                    <div style={{ width: 32, height: 32, borderRadius: 9, background: 'color-mix(in srgb, #2563eb 12%, var(--background))', color: '#2563eb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <BarChart2 size={14} />
                    </div>
                    <div>
                ...
                <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        {activeTab === 'transactions' && (
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', margin: isMobile ? '1rem' : 0 }}>
...
const StatCards: React.FC<{
  cards: StatCard[]; isMobile: boolean;
  fmtEur: (n: number) => string; isBalanceHidden: boolean;
}> = ({ cards, isMobile, fmtEur, isBalanceHidden }) => (
  <div style={{
    display: 'grid',
    ...
    {cards.map(({ label, value, icon, iconColor, iconBg }, idx) => (
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{
            fontSize: isMobile ? '0.95rem' : '1.2rem', fontWeight: 700, margin: 0,
            letterSpacing: isBalanceHidden ? '0.04em' : '-0.02em',
            color: (label === 'Cash' || label === 'Meal Vouchers') ? iconColor : 'var(--foreground)',
...
const BalanceSplitCard: React.FC<{
  cashBalance: number; bpBalance: number;
  fmtEur: (n: number) => string; isBalanceHidden: boolean;
}> = ({ cashBalance, bpBalance, fmtEur, isBalanceHidden }) => (
  <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
      {[
        ...
        { label: 'Meal Vouchers', value: bpBalance,   color: bpBalance   >= 0 ? '#d97706' : '#dc2626', emoji: '🎟️' },
        ...
        <div key={label} style={{ background: 'var(--accent)', borderRadius: '12px', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          <span style={{ fontSize: '0.65rem', fontWeight: 600, color: value >= 0 ? '#16a34a' : '#dc2626' }}>
...
const MobileSavingsCard: React.FC<{
  ...
  fmtEur: (n: number) => string; isBalanceHidden: boolean;
}> = ({ income, expenses, savings, fmtEur, isBalanceHidden }) => {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '3px 9px', borderRadius: '999px', background: isPositive ? 'color-mix(in srgb, #16a34a 12%, var(--background))' : 'color-mix(in srgb, #dc2626 12%, var(--background))', color: isPositive ? '#16a34a' : '#dc2626' }}>
      ...
      <div style={{ background: 'var(--muted)', borderRadius: '999px', height: 8, overflow: 'hidden', marginBottom: '0.75rem' }}>
        <div style={{ height: '100%', borderRadius: '999px', width: `${spentPct}%`, background: spentPct > 80 ? 'linear-gradient(90deg, #fca5a5, #ef4444)' : 'linear-gradient(90deg, #86efac, #22c55e)', transition: 'width 0.5s ease' }} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem' }}>
        {[
          { label: 'Income',   value: income,   color: '#16a34a' },
          { label: 'Spent',    value: expenses, color: '#ef4444' },
          { label: 'Invested', value: savings,  color: '#7c3aed' },
