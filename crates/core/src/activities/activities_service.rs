...
impl ActivityServiceTrait for ActivityService {
    ...
    async fn repair_activities_missing_currency(&self, activity_ids: Vec<String>) -> Result<u32> {
        let blank: Vec<Activity> = self
            .activity_repository
            .get_activities_by_ids(&activity_ids)?
            .into_iter()
            .filter(|activity| activity.currency.trim().is_empty())
            .collect();
        if blank.is_empty() {
            return Ok(0);
        }

        let unique_account_ids: HashSet<String> = blank
            .iter()
            .map(|activity| activity.account_id.clone())
            .collect();
        let mut account_currency_by_id: HashMap<String, String> = HashMap::new();
        for account_id in unique_account_ids {
            let account = self.account_service.get_account(&account_id)?;
            if !account.currency.trim().is_empty() {
                account_currency_by_id.insert(account_id, account.currency);
            }
        }

        let mut repairs: Vec<ActivityCurrencyRepair> = Vec::with_capacity(blank.len());
        let mut account_ids: HashSet<String> = HashSet::new();
        let mut asset_ids: HashSet<String> = HashSet::new();
        let mut currencies: HashSet<String> = HashSet::new();
        ...
        for activity in &blank {
            let Some(currency) = account_currency_by_id.get(&activity.account_id) else {
                // The account has no currency to copy; leave the row flagged so
                // the health check keeps surfacing it for manual repair.
                continue;
            };

            // Only CSV imports write an idempotency key derived from the
            // currency. Recompute it with the repaired currency so re-importing
            // the same file matches this row instead of inserting a duplicate.
            // The stored asset UUID is what a re-import would key on now that
            // the asset exists, so recomputing from the stored row is exact.
            let idempotency_key = (activity.source_system.as_deref() == Some("CSV")
                && activity.idempotency_key.is_some())
            .then(|| {
            ...
            earliest_at = match earliest_at {
                Some(current) if current <= activity.activity_date => Some(current),
                _ => Some(activity.activity_date),
            };
            repairs.push(ActivityCurrencyRepair {
        ...
        let repaired = self
            .activity_repository
            .repair_activity_currencies(repairs)
            .await?;
        if repaired > 0 {
            self.emit_activities_changed(
                account_ids.into_iter().collect(),
